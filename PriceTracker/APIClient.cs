...
namespace PriceTracker
{
    public class APIClient
	{
		...
		private TimeSpan? _priceInterval;
		...
		public APIClient(string url = DEFAULT_URL, string priceKey = DEFAULT_PRICEKEY, string currPair = DEFAULT_CURRPAIR, TimeSpan? priceInterval = null)
		{
			this._url = url;
			this._priceInterval = (priceInterval == null ? DEFAULT_PRICEINTERVAL : priceInterval);
        ...
        public async void StartProcessAsync()
        {
			...
			Console.WriteLine("Starting with interval of {0} seconds {1}", this._priceInterval.Value.TotalSeconds, Environment.NewLine);
